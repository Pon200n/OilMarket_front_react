  ...
  return (
    <div>
      <div className="wrapper_basket">
        ...
        <div className="basket_price">{priceForm.format(props.p.price)} ₽</div>

        <div className="basket_quantity">
          <div className="basket_quantity_inner">
            <button className="decrement" onClick={props.dicr}>
              -
            </button>
            <input
              type={"number"}
              value={props.p.count}
              onChange={(event) => chengeValue(props.p.id, event.target.value)}
              className="quantity_input"
            ></input>
            <button className="increment" onClick={props.incr}>
              +
            </button>
          </div>
        </div>
        ...
        <div className="basket_button_delete">
          <button className="button_delete" onClick={props.deleteFromBasket}>
            X Удалить
          </button>
        </div>
      </div>
